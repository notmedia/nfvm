...
class Pack {
  ...
  async createFile(filename, global = false) {
    const data = await utils.prompt.file.create(filename, global);
  ...
  async createFileVersion(filename, version, global = false) {
    const file = utils.find.file(this.files, filename, global);
  ...
  async setVersion(version) {
    ...
    await Bluebird.map(this.files, file =>
      file.setVersion(version, this.removeFileIfVersionNotExists));
  }
