...
class Pack {
  ...
  async createFileVersion(filename, version, global = false) {
    ...
    const data = await utils.prompt.file
      .createVersion(version, file.versions.map(item => item.name), global);
  ...
  setVersion(version) {
    ...
    const tasks = new Listr(this.files.map(file => ({
      title: `${file.filename} ${chalk.cyan('>>>')} ${version} ${chalk.cyan('>>>')} ${file.destinationPath}`,
      task: () => file.setVersion(version),
    })));
